...
server <- function(input, output, session) {
  ...
  observeEvent(input$enregistrer, {
    ...
    # 1) requested: auto-clear after save
    clear_form()
  })
  
  # =========================================================
  # Clear form button (kept, but now uses helper)
  # =========================================================
  ...
  observeEvent(input$editer, {
    ...
    # marker for editing + store snapshot for discard
  ...
  # Also supports multi-select and deleting directly from table
  # =========================================================
  observeEvent(input$supprimer, {
    
    onglet <- input$onglets
    
    # Prefer selection in current tab; fallback to loaded-for-edit event
  ...
  observeEvent(input$confirm_soft_delete, {
    ...
    tryCatch({
      ...
      # reset edit marker if it was among deleted
      if (!is.null(session$userData$indice_edition)) {
        idx <- session$userData$indice_edition
  ...
  observeEvent(input$confirm_hard_delete, {
    ...
    tryCatch({
      ...
      # delete uploaded images folders for those events
      for (id_evt in ids) {
        dir_evt <- file.path("Graphs_uploaded", id_evt)
  ...
  # --- Buttons impacting the currently active tab (NOW includes EIOS + Deleted) ---
  ...
  # =========================================================
  # Word report generation (your existing functions kept)
  # =========================================================
